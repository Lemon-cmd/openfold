...
class OpenFoldDataLoader(torch.utils.data.DataLoader):
    ...
    def _prep_batch_properties_probs(self):
        keyed_probs = []
        stage_cfg = self.config.data[self.stage]

        max_iters = self.config.data.common.max_recycling_iters
        if(stage_cfg.supervised):
            clamp_prob = self.config.data.supervised.clamp_prob
            keyed_probs.append(
                ("use_clamped_fape", [1.0 - clamp_prob, clamp_prob])
            )
...
class OpenFoldDataModule(pl.LightningDataModule):
    ...
    def setup(self):
        ...
        if(self.training_mode):
            train_dataset = dataset_gen(
                ...
                max_template_hits=self.config.data.train.max_template_hits,
                shuffle_top_k_prefiltered=
                    self.config.data.train.shuffle_top_k_prefiltered,
            ...
            if(self.distillation_data_dir is not None):
                ...
                d_prob = self.config.data.train.distillation_prob
            ...
            self.predict_dataset = dataset_gen(
                ...
                mapping_path=None,
                max_template_hits=self.config.data.predict.max_template_hits,
                mode="predict",
            )
    ...
    def _gen_dataloader(self, stage):
        ...
        dl = OpenFoldDataLoader(
            ...
            stage=stage,
            generator=generator,
            batch_size=self.config.data.data_module.data_loaders.batch_size,
            num_workers=self.config.data.data_module.data_loaders.num_workers,
